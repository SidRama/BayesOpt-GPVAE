...
def minibatch_sgd(covar_module, likelihood, latent_dim, m, H, train_xt, mu, log_v, z, P_batch, natural_gradient, eps, N):

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    x_st = torch.reshape(train_xt, [P_batch, train_xt.shape[1]]).to(device)
    stacked_x_st = torch.stack([x_st for i in range(latent_dim)], dim=0)
#    print(train_xt.shape)
#    print(z.shape)
#    print(covar_module)
    M = H.shape[-1]
    K0xz = covar_module(train_xt, z).evaluate()
    K0zz = covar_module(z, z).evaluate()
    K0zz = K0zz + eps * torch.eye(M, dtype=torch.double).to(device)
    LK0zz = torch.cholesky(K0zz)
    iK0zz = torch.cholesky_solve(torch.eye(M, dtype=torch.double).to(device), LK0zz)
    sigma_z_inv = 1/likelihood.noise_covar.noise.to(device)
    LH = torch.cholesky(H)
    iH = torch.cholesky_solve(torch.eye(M, dtype=torch.double).to(device), LH)

    K0xx = covar_module(stacked_x_st, stacked_x_st).evaluate()

    B_st = torch.eye(P_batch, dtype=torch.double).to(device) * likelihood.noise_covar.noise.unsqueeze(dim=2).to(device)

    LB_st = torch.cholesky(B_st).to(device)
    iB_st = torch.cholesky_solve(torch.eye(P_batch, dtype=torch.double).to(device), LB_st)

 #   K0xz_st = torch.reshape(K0xz, [latent_dim, P_batch, T, M])
    iB_K0xz = torch.matmul(iB_st, K0xz)
    K0zx_iB_K0xz = torch.matmul(torch.transpose(K0xz, -1, -2), iB_K0xz)

    iK0zz_K0xz = torch.matmul(iK0zz, K0xz.transpose(1, 2))
#    print(K0xz.shape)
#    print(iB_K0xz.shape)
#    print(K0xx.shape)

    K0_tilde = K0xx - torch.matmul(K0xz, iK0zz_K0xz)

    _ = (torch.matmul(torch.matmul(K0xz, iK0zz), m).squeeze() - mu.T).reshape(latent_dim, P_batch, -1)
    # A = sigma_z_inv * torch.matmul(_.transpose(1, 2), _).sum()
    # B = sigma_z_inv * torch.exp(log_v).sum()
    # C = sigma_z_inv * torch.diagonal(K0_tilde, dim1=-1, dim2=-2).sum()
    # _ = torch.matmul(torch.matmul(iK0zz, H), iK0zz)
    # K0zxK0xz = torch.matmul(K0xz.transpose(-1, -2), K0xz)
    # D = sigma_z_inv * torch.diagonal(torch.matmul(_, K0zxK0xz), dim1=-1, dim2=-2).sum()
    # E = torch.sum(log_v)

    A = torch.matmul(_.transpose(1, 2), _).sum()
    B = torch.exp(log_v).sum()
    C = torch.diagonal(K0_tilde, dim1=-1, dim2=-2).sum()
    _ = torch.matmul(torch.matmul(iK0zz, H), iK0zz)

    K0zxK0xz = torch.matmul(K0xz.transpose(-1, -2), K0xz)

    D = torch.diagonal(torch.matmul(_, K0zxK0xz), dim1=-1, dim2=-2).sum()
    E = torch.sum(log_v)
    ...
    kld_total = (N/(2 * P_batch)) * (A + B + C + D - E) - (latent_dim * N*0.5) + kld_qu_pu
    # # Compute gradients of m and H w.r.t. kld_total
    #
    # grad_m, grad_H = None, None
    # if natural_gradient:
    #     mu = mu.transpose(0, 1).reshape(latent_dim, P_batch, -1)
    #     K0zx = K0xz.transpose(-1, -2)
    #     A = torch.matmul(torch.matmul(iK0zz.unsqueeze(dim=1), K0zx), mu).sum(dim=1)
    #     B = torch.matmul(torch.matmul(iK0zz, K0zxK0xz), iK0zz) + iK0zz
    #
    #     grad_m = -A + torch.matmul(B, m)
    #     grad_H = 0.5 * (-iH + B)
...
#    K0zx = K0xz.transpose(-1, -2)
    ...
    if natural_gradient:
        mu = mu.transpose(0, 1).reshape(latent_dim, P_batch, -1)
        K0zx = K0xz.transpose(-1, -2)
        A = torch.matmul(torch.matmul(iK0zz, K0zx), torch.matmul(iB_st, mu))
        B = torch.matmul(torch.matmul(iK0zz, K0zx_iB_K0xz), iK0zz) + iK0zz
        grad_m = -A + torch.matmul(B, m)
        grad_H = 0.5 * (-iH + B)
